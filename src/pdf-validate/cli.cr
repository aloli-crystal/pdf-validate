...
# Callable CLI for the PDF conformance validator. Kept as a module method
# (rather than top-level code in `src/cli.cr`) so it can run both as the
# standalone `pdf-validate` binary AND in-process from the unified
# `alolipdf` binary (aloli-crystal/pdf-tools).
#
