  ...
  module Validate
    ...
    module Checks
      ...
      def self.evaluate(check : String, args : Array(String), ctx : Context) : Outcome
        ...
        when "page_transparency_groups_valid"
          bad = ctx.page_transparency_group_violations
          Outcome.new(
            bad.empty?,
            bad.empty? ? nil : bad.join(", ")
          )
        ...
        when "cmaps_valid"
          bad = ctx.cmap_violations
          Outcome.new(
            bad.empty?,
            bad.empty? ? nil : bad.join(", ")
          )
        when "truetype_encodings_valid"
          bad = ctx.truetype_encoding_violations
          Outcome.new(
            bad.empty?,
            bad.empty? ? nil : bad.join(", ")
          )
        ...
        when "referenced_glyphs_present"
          bad = ctx.referenced_glyph_absent_violations
          Outcome.new(
            bad.empty?,
            bad.empty? ? nil : bad.join(", ")
          )
        when "no_notdef_references"
          bad = ctx.notdef_reference_violations
          Outcome.new(
            bad.empty?,
            bad.empty? ? nil : bad.join(", ")
          )
